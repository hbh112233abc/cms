...
class Sign extends \app\BaseController
{
    ...
    public function login()
    {
        ...
        $result = $this->validate(input('param.'), '\\app\\common\\validate\\User.login');
        if (true !== $result) {
        ...
        if (request()->isAjax()) {
            ...
            if ($user) {
                ...
                if ($res) {
                    ...
                    return $this->success('注册成功, 邮件发送失败!', url(request()->module() . '/Sign/login'), ['uid' => $user['user_id'], 'email' => $user['email']]);
                }
            } else {
                return $this->error($userLogic->getError());
            }
