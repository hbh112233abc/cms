...
class Webmaster
{
    ...
    public function checkIndex(Job $job, $data)
    {
        ...
        //$url = url('index/Article/viewArticle', ['aid' => $aid], true, get_config('domain_name')); //job中使用url，获取异常
        //$url = get_config('domain_name') . url('index/Article/viewArticle', ['aid' => $id], true, false); //hack
        $url = $data['url'];
