...
class Sitemap extends Base
{
    ...
    public function xml($id = '')
    {
        ...
        //生成index 首页
        $sitemap->addItem(url('index/Index/index', null, false, get_config('domain_name')), 1, "hourly", date_time());
        $sitemap->addItem(url('index/Index/about', null, false, get_config('domain_name')), 1, "monthly", date_time());
        $sitemap->addItem(url('index/Index/contact', null, false, get_config('domain_name')), 1, "monthly", date_time());
        $sitemap->addItem(url('index/Index/about', null, false, get_config('domain_name')), 1, "monthly", date_time());
        ...
        foreach ($resultSet as $category) {
            ...
            $loc = url('index/Article/articleList', ['cid' => $category->id], false, get_config('domain_name'));
            $sitemap->addItem($loc, $priority, "daily", date_time());

            $loc = url('index/Article/articleList', ['cname' => $category->title_en], false, get_config('domain_name'));
            $sitemap->addItem($loc, $priority, "daily", date_time());
        }
        ...
        foreach ($resultSet as $article) {
            ...
            $loc = url('index/Article/viewArticle', ['aid' => $article->id], false, get_config('domain_name'));
        ...
        //生成sitemap index;
        $sitemapLoc = url('index/Sitemap/xml', null, false, get_config('domain_name'));
    ...
    {
        $templateFile = Env::get('app_path') . 'index' . DIRECTORY_SEPARATOR . 'view' . DIRECTORY_SEPARATOR . 'sitemap' . DIRECTORY_SEPARATOR . 'sitemap.html';
